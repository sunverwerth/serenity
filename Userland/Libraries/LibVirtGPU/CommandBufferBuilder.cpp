...
void CommandBufferBuilder::append_draw_vbo(u32 count, Protocol::PipePrimitiveTypes primitive_type)
{
...
void CommandBufferBuilder::append_gl_clear(float r, float g, float b, float a)
{
    ...
    builder.appendf32(r); // Red
    builder.appendf32(g); // Green
    builder.appendf32(b); // Blue
    builder.appendf32(a); // Alpha
    builder.appendf64(0);  // Depth
    builder.appendu32(0); // Stencil
}

void CommandBufferBuilder::append_gl_clear(float d)
{
    CommandBuilder builder(m_buffer, Protocol::VirGLCommand::CLEAR, 0);
    Protocol::ClearType clear_flags {};
    clear_flags.flags.depth = 1;
    builder.appendu32(clear_flags.value);
    builder.appendf32(0); // Red
    builder.appendf32(0); // Green
    builder.appendf32(0); // Blue
    builder.appendf32(0); // Alpha
    builder.appendf64(d); // Depth
    builder.appendu32(0); // Stencil
}
...
void CommandBufferBuilder::append_gl_viewport(int width, int height)
{
    CommandBuilder builder(m_buffer, Protocol::VirGLCommand::SET_VIEWPORT_STATE, 0);
    builder.appendu32(0);
    builder.appendf32(width / 2);    // scale_x
    builder.appendf32(height / 2); // scale_y (flipped, due to VirGL being different from our coordinate space)
    builder.appendf32(0.5f);                    // scale_z
    builder.appendf32(width / 2);    // translate_x
    builder.appendf32(height / 2);   // translate_y
    builder.appendf32(0.5f);                    // translate_z
}

void CommandBufferBuilder::append_set_framebuffer_state_no_attach(int width, int height)
{
    CommandBuilder builder(m_buffer, Protocol::VirGLCommand::SET_FRAMEBUFFER_STATE_NO_ATTACH, 0);
    builder.appendu32((height << 16) | width); // (height << 16) | width
